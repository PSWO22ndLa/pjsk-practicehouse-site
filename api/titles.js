...
function loadTitles() {
  try {
    if (fs.existsSync(titlesPath)) {
      ...
      console.log('[Titles API] 成功載入:', Object.keys(parsed).length, '位使用者');
      return parsed;
    } else {
      console.log('[Titles API] titles.json 不存在，建立新檔案');
      fs.writeFileSync(titlesPath, '{}', 'utf8');
      return {};
    }
  } catch (error) {
    console.error('[Titles API] 讀取失敗:', error);
    return {};
  }
}
...
function saveTitles(data) {
  try {
    ...
    console.log('[Titles API] 成功儲存');
  } catch (error) {
    console.error('[Titles API] 儲存失敗:', error);
  }
}
...
module.exports = (req, res) => {
  ...
  console.log(`[Titles API] ${req.method} ${action} - UserId: ${userId}`);
  ...
  if (req.method === 'GET' && action === 'get') {
    ...
    console.log('[Titles API] 使用者資料:', userData);
    
    if (!userData) {
      console.log('[Titles API] 使用者不存在，返回空資料');
      return res.json({ 
        success: true,
        titles: [],
  ...
  if (req.method === 'POST' && action === 'equip') {
    ...
    console.log('[Titles API] 裝備稱號:', { titleId, slot });
    ...
    if (!userData.specialTitles || !userData.specialTitles.includes(titleId)) {
      console.log('[Titles API] 使用者沒有此稱號');
    ...
    console.log('[Titles API] 裝備成功:', userData.equippedTitles);
  ...
  if (req.method === 'POST' && action === 'unequip') {
    ...
    console.log('[Titles API] 卸下稱號:', { slot });
    ...
    console.log('[Titles API] 卸下成功:', userData.equippedTitles);
